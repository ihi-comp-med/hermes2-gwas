...
def ldsc_get_n(wc):
    ...
    samp_prev = N_case / (N_case + N_control)
    return {'N_case': N_case, 'N_control': N_control, 'samp_prev': samp_prev}
...
rule ldsc_munge_sumstats:
    input:
        ...
        snplist = lambda x: load_config(x)['ldsc']['hm3_snplist'],
        script = "scripts/ldsc/munge_sumstats.py"
    output:
        "results/ldsc/{analysis_id}/ldsc.sumstats.gz"
    conda:
        "envs/ldsc.yaml"
    params:
        prefix = lambda x: f"results/ldsc/{x.analysis_id}/ldsc",
        N = ldsc_get_n,
        chunksize = 500000
    shell:
        """
...
rule ldsc_calc_h2:
    """Run LDSC to calculate h2 and intercept for population stratification
    Note: Only intercept calculation is used (we use LDAK for h2 analysis)
    """
    input:
        sumstats = rules.ldsc_munge_sumstats.output,
        dir_ld_chr = lambda x: load_config(x)['ldsc']['dir_ldsc_chr'],
        script = "scripts/ldsc/ldsc.py"
    output:
        "results/ldsc/{analysis_id}/ldsc_h2.log"
    params:
        prefix = lambda x, output: strip_ext(x, output[0]),
        samp_prev = lambda x: ldsc_get_n(x)['samp_prev'],
